...
def parse_protocol(protocol):
    ...
    for section in iterate_sections(protocol_parsed, h2_indices):
        ...
        topic_indices = np.where(
            [x["type"] == "heading" and x["level"] == 3 for x in section]
        )[0]
...
def main(client: DiscourseStorageClient) -> None:
    ...
    if not pad_links:
        logging.error(
            f'The first post in "{title}" does not contain a pad link. Aborting.'
        )
        return

    if len(pad_links) > 1:
        logging.error(
            f'The first post in "{title}" does contains multiple pad links. Aborting.'
        )
        return
    ...
    protocol_posted_content = render(
        "protocol_posted.md",
