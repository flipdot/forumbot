...
def get_next_plenum_date(now: datetime):
...
def announce_plenum(client: DiscourseClient):

    now = datetime.now()
    plenum_date, delta = get_next_plenum_date(now)
    if delta.days > 7:
        logging.info(f'Next plenum is too far in the future ({delta.days} days). Aborting.')
        return
    title = plenum_date.strftime('%Y-%m-%d Plenum')
    topics = [x['title'] for x in client.category_topics('orga/plena')['topic_list']['topics']]
    if title in topics:
